...
def getKnownCharacter(databasePath, ID):
    DBConnection = sqliteEngine.sqliteEngine(databasePath)
    result = DBConnection.readEntryFiltered("Information", "KnownCharacter", f"ID = {ID}")
    DBConnection.commitClose()
    return result[0][0]

def getAboutCharacter(databasePath, ID):
    DBConnection = sqliteEngine.sqliteEngine(databasePath)
    result = DBConnection.readEntryFiltered("Information", "AboutCharacter", f"ID = {ID}")
    DBConnection.commitClose()
    return result[0][0]

def getDescription(databasePath, ID):
    DBConnection = sqliteEngine.sqliteEngine(databasePath)
    result = DBConnection.readEntryFiltered("Information", "Description", f"ID = {ID}")
    DBConnection.commitClose()
    return result[0][0]
