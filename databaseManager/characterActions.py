...
def editPlayer(databasePath, characterID, player):
    DBConnection = sqliteEngine.sqliteEngine(databasePath)
    DBConnection.updateEntry("Characters", "Player", player, f"ID = {characterID}")
    DBConnection.commitClose()

def editCharacteristics(databasePath, characterID, strength, medicine, hacking):
    DBConnection = sqliteEngine.sqliteEngine(databasePath)
    DBConnection.updateEntry("Characters", "Strength", strength, f"ID = {characterID}")
    DBConnection.updateEntry("Characters", "Medicine", medicine, f"ID = {characterID}")
    DBConnection.updateEntry("Characters", "Hacking", hacking, f"ID = {characterID}")
    DBConnection.commitClose()
...
def editMainObjective(databasePath, characterID, mainObjective):
    DBConnection = sqliteEngine.sqliteEngine(databasePath)
    DBConnection.updateEntry("Characters", "MainObjective", mainObjective, f"ID = {characterID}")
    DBConnection.commitClose()
    
def editSecondaryObjective(databasePath, characterID, secondaryObjective):
    DBConnection = sqliteEngine.sqliteEngine(databasePath)
    DBConnection.updateEntry("Characters", "SecondaryObjective", secondaryObjective, f"ID = {characterID}")
    DBConnection.commitClose()
    
def editLoseCondition(databasePath, characterID, loseCondition):
    DBConnection = sqliteEngine.sqliteEngine(databasePath)
    DBConnection.updateEntry("Characters", "LoseCondition", loseCondition, f"ID = {characterID}")
    DBConnection.commitClose()
    
def editFullCharacter(databasePath, characterID, name, player, strength, medicine, hacking, background, mainObjective, secondaryObjective, loseCondition):
    DBConnection = sqliteEngine.sqliteEngine(databasePath)
    DBConnection.updateEntry("Characters", "Name", name, f"ID = {characterID}")
    DBConnection.updateEntry("Characters", "Player", player, f"ID = {characterID}")
    DBConnection.updateEntry("Characters", "Strength", strength, f"ID = {characterID}")
