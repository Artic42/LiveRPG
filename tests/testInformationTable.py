...
def test11_13():
    ...
    test.testIfEqual(1, DB.readNumberOfEntries("Information"),
                     "Checking if information has been created")
    DB.commitClose()
    infoActions.createInformation("test.db", 2)
    DB = sqliteEngine.sqliteEngine("test.db")
    test.testIfEqual(2, DB.readNumberOfEntries("Information"),
                     "Checking if a second information has been created")
    DB.commitClose()
    infoActions.deleteInformation("test.db", 1)
    DB = sqliteEngine.sqliteEngine("test.db")
    test.testIfEqual(1, DB.readNumberOfEntries("Information"),
                     "Checking if a information has been deleted")
    DB.commitClose()
    infoActions.createInformation("test.db", 1)
...
def test21_29():
    ...
    test.testIfEqual("Description2", infoConsults.getDescription("test.db", 2),
                     "Get description")
