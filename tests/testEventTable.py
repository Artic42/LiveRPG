...
def test11_13():
    ...
    test.testIfEqual(1, DB.readNumberOfEntries("Events"),
                     "Checking if event has been created")
    DB.commitClose()
    eventActions.createEvent("test.db", 2)
    DB = sqliteEngine.sqliteEngine("test.db")
    test.testIfEqual(2, DB.readNumberOfEntries("Events"),
...
def test21_29():
    ...
    test.testIfEqual("Description2",
                     eventConsults.getEventDescription("test.db", 2),
                     "Get description")
    test.testIfTrue(eventConsults.getEventActivated("test.db", 2),
                    "Get activated")
    test.testIfEqual(3, eventConsults.getEventRedirectID("test.db", 2),
                     "Get redirect ID")
    ...
    test.testIfEqual("Description1",
                     eventConsults.getEventDescription("test.db", 1),
                     "Get description")
    test.testIfTrue(eventConsults.getEventActivated("test.db", 2),
                    "Get activated")
