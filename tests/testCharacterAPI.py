...
def test11():
    ...
    test.testIfEqual(characters["1"], "Name1",
                     "Characters request character name")
    test.testIfEqual(characters["2"], "Name2",
                     "Characters request character name")
...
def test13():
    ...
    test.testIfEqual(characters.json()["status"], 200,
...
def test24():
    ...
    test.testIfEqual(background.json()["background"],
                     "NewBackground1", "Read background")


def test25():
    response = characterRequest.editMainObjective(1, "NewMainObjective1")
    test.testIfEqual(response.json()["status"], 200,
                     "Edit main objective status")
    mainObjective = characterRequest.readMainObjective(1)
    test.testIfEqual(mainObjective.json()["mainObjective"],
                     "NewMainObjective1", "Read main objective")


def test26():
    response = characterRequest.editSecondaryObjective(
        1, "NewSecondaryObjective1")
    test.testIfEqual(response.json()["status"], 200,
                     "Edit secondary objective status")
    secondaryObjective = characterRequest.readSecondaryObjective(1)
    test.testIfEqual(secondaryObjective.json()["secondaryObjective"],
                     "NewSecondaryObjective1", "Read secondary objective")


def test27():
    response = characterRequest.editLoseCondition(1, "NewLoseCondition1")
    test.testIfEqual(response.json()["status"], 200,
                     "Edit lose condition status")
    loseCondition = characterRequest.readLoseCondition(1)
    test.testIfEqual(loseCondition.json()["loseCondition"],
                     "NewLoseCondition1", "Read lose condition")
...
def test32():
    ...
    test.testIfEqual(character.json()["Background"],
                     "NewBackground2", "Read background")
    test.testIfEqual(character.json()["MainObjective"],
                     "NewMainObjective2", "Read main objective")
    test.testIfEqual(character.json()["SecondaryObjective"],
                     "NewSecondaryObjective2", "Read secondary objective")
    test.testIfEqual(character.json()["LoseCondition"],
